...
# Candidate pool for the next independent holdout. Before any public-site inspection,
# --validate-only proves that none of these hosts occur in the versioned PRV-103 QA/
# calibration history. The pool is then frozen in a GitHub Issue without replacements.
POOL = [
...
# Issue #221 was already exposed to development/QA and therefore is excluded even
# if a host is not present in versioned repository history.
...
def main() -> None:
    ...
    for site_index, requested_url in enumerate(POOL, start=1):
        ...
        for form_index, form in enumerate(high_forms, start=1):
            ...
            # Artifact A deliberately contains no URL, site identity, product prediction,
            # field names outside the approved four fields, or diagnostic result.
            blind_forms.append({
                "blind_id": blind_id,
