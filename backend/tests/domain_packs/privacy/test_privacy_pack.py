...
def evaluated_scenario():
    ...
    for code, evidence in {
        "PRV-005": {"controller_identification": "clear"},
        "PRV-006": {"rights_channel": "explicit"},
        "PRV-007": {"data_categories": "explicit"},
        "PRV-008": {"processing_purposes": "explicit"},
        "PRV-011": {"holder_rights": "multiple"},
    }.items():
        results[code] = evaluate_control(code, evidence, results)
...
@pytest.mark.parametrize("code", ("PRV-005", "PRV-006", "PRV-007", "PRV-008", "PRV-011"))
def test_b1_substantive_controls_follow_prv003_gate(code):
...
@pytest.mark.parametrize(("code", "key", "values"), [
    ("PRV-005", "controller_identification", (("clear", "detected"), ("generic", "partial"), ("none", "not_detected"))),
    ("PRV-006", "rights_channel", (("explicit", "detected"), ("generic", "partial"), ("none", "not_detected"))),
    ("PRV-007", "data_categories", (("explicit", "detected"), ("generic", "partial"), ("none", "not_detected"))),
    ("PRV-008", "processing_purposes", (("explicit", "detected"), ("generic", "partial"), ("none", "not_detected"))),
    ("PRV-011", "holder_rights", (("multiple", "detected"), ("limited", "partial"), ("none", "not_detected"))),
])
def test_b1_substantive_control_states(code, key, values):
    for signal, expected in values:
        result = evaluate_control(code, {key: signal}, {"PRV-003": "detected"})
...
def test_not_evaluable_is_unscored_and_reduces_coverage():
    ...
    assert scored["evaluated_controls"] == 10
    assert scored["applicable_controls"] == 11
    assert scored["coverage"] == 91
...
def test_integral_scenario_returns_the_approved_result():
    assert score_privacy(evaluated_scenario().values()) == {
        ...
        "status": "Alta preparación visible",
        "coverage": 100,
        "evaluated_controls": 11,
        "applicable_controls": 11,
    }
