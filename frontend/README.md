...
- `MININODE_API_BASE`: URL base del backend (Render). El front tambi�n la usa directamente cuando corre como file://; si no se define, usa `/api` (proxy) en producci�n.
- `MININODE_API_KEY`: secret en Pages; el proxy lo reenv�a como `X-Api-Key` al backend.
- Def�nelas en Cloudflare Pages ? Project ? Settings ? Environment Variables / Secrets.

## API Proxy (Functions)
- Ruta: `frontend/functions/api/[[path]].js`.
- Lista blanca: `ALLOWED`.
- Mapping p�blico ? backend: `ROUTE_MAP`.
- Rutas actuales: `write/draft`, `analyze/summary`, `capture`.
- Maneja CORS y soporta `multipart/form-data` (reenv�a el stream con boundary intacto).

## Write Page
- Location: `frontend/agents/write/index.html` con l�gica en `frontend/assets/js/write.page.js`.
- Endpoints usados (v�a proxy): `/api/write/draft`, `/api/analyze/summary`.
- Notas:
  - Borrador local funciona sin backend.
  - El an�lisis de URLs requiere backend/API key para resultados reales.

## Capture (UI)
- Formats: `jpg`, `jpeg`, `png` (= 5 MB). `webp` se convierte en cliente. `heic/heif` bloqueado con aviso.
- Feedback inmediato al elegir/soltar: miniatura, nombre, tama�o; bot�n se habilita.
- Badge: `Total Xs | p95(sess) Ys` (p95 local de la sesi�n del navegador).
- Differences: checks fallidos antes/despu�s y campos ajustados cuando hay fallback.

## Run With Proxy Locally
- With Cloudflare Pages Functions:
  - `wrangler pages dev frontend` para correr est�tico + functions.
  - Define `MININODE_API_BASE` y `MININODE_API_KEY` si el backend exige auth.

## Build & Deploy
- Est�tico: deploy `frontend/` en tu host.
...
- Mixed content: usa HTTPS para sitio y API en producci�n.
